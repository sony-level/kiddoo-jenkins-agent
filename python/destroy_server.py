...
def parse_args():
    p = argparse.ArgumentParser(
        description="Destroys all kiddoo-jenkins-agent AWS resources."
    )
...
def main():
    ...
    notify(
        "kiddoo-jenkins-agent: cleanup started",
        f"Destroying all resources in {args.region}.",
        INFO,
    )
    ...
    try:
        ...
        notify(
            "kiddoo-jenkins-agent: cleanup failed",
            f"Script exited with code {e.returncode}.",
            ERR,
        )
    ...
    notify(
        "kiddoo-jenkins-agent: cleanup complete",
